...
#[AllowDynamicProperties]
 class Xcrud implements \Stringable
 {
     ...
     public static function get_instance($name = false)
     {
         self::init_prepare();
         if (!$name)
             $name = sha1(random_int(0, mt_getrandmax()) . microtime());
     ...
     {
         $sess_name = match ($method) {
             'post' => (Xcrud_config::$dynamic_session && isset($_POST['xcrud']['sess_name']) && $_POST['xcrud']['sess_name']) ? $_POST['xcrud']['sess_name'] :
                 Xcrud_config::$sess_name,
             'get' => (Xcrud_config::$dynamic_session && isset($_GET['xcrud']['sess_name']) && $_GET['xcrud']['sess_name']) ? $_GET['xcrud']['sess_name'] :
                 Xcrud_config::$sess_name,
             default => Xcrud_config::$sess_name,
         };
     ...
     public static function session_start($sess_name = false)
     {
         ...
         if (Xcrud_config::$alt_session)
         {
             if (!headers_sent())
             {
                 ...
                 {
                     self::$sess_id = base_convert(str_replace(' ', '', microtime()) . random_int(0, mt_getrandmax()), 10, 36);
                 }
                 else
                 {
                     self::$sess_id = $_COOKIE[$sess_name];
                 }
                 setcookie($sess_name, (string) self::$sess_id, ['expires' => time() + Xcrud_config::$alt_lifetime * 60, 'path' => '/']);
             }
             else
     ...
     public function order_by($fields = '', $direction = 'asc')
     {
         if ($fields)
         {
             ...
             else
             {
                 ...
                 {
                     $this->order_by[$key] = $fitem['value'] ?? $direction;
                 }
             }
         }
         unset($fields);
         ...
         if ($fields && $rel_tbl && $rel_field && $rel_name)
         {
             ...
             foreach ($fdata as $fitem)
             {
                 $this->relation[$fitem['table'] . '.' . $fitem['field']] = array(
                     'rel_tbl' => $rel_tbl,
                     'rel_alias' => 'alias' . random_int(0, mt_getrandmax()),
                     'rel_field' => $rel_field,
                     'rel_name' => $rel_name,
                     'rel_where' => $rel_where,
         ...
         array $add_data = array())
     {
         if ($fields && $rel_tbl && $rel_field && $rel_name && $label)
         {
             ...
             $this->fk_relation[$alias] = array(
                 ...
                 'alias' => $alias,
                 'rel_alias' => 'ralias' . random_int(0, mt_getrandmax()),
                 'fk_alias' => 'fkalias' . random_int(0, mt_getrandmax()),
                 'fk_table' => $fk_table,
     ...
     public function label($fields = '', $label = '')
     {
         ...
         foreach ($fdata as $fitem)
         {
             $this->labels[$fitem['table'] . '.' . $fitem['field']] = $fitem['value'] ?? $label;
         }
         return $this;
     }
     ...
     public function pass_default($fields = '', $value = '')
     {
         ...
         foreach ($fdata as $fitem)
         {
             $this->defaults[$fitem['table'] . '.' . $fitem['field']] = $fitem['value'] ?? $value;
         }
         return $this;
     }
     ...
     public function links_label($text = '')
     {
         if ($text)
         {
             $this->links_label['text'] = trim((string) $text);
         }
         return $this;
     }
     public function emails_label($text = '')
     {
         if ($text)
         {
             $this->emails_label['text'] = trim((string) $text);
         }
         return $this;
     }
     ...
     public function validation_required($fields = '', $chars = 1)
     {
         ...
         foreach ($fdata as $fitem)
         {
             $this->validation_required[$fitem['table'] . '.' . $fitem['field']] = $fitem['value'] ?? $chars;
         }
         return $this;
     }
     public function validation_pattern($fields = '', $pattern = '')
     {
         ...
         foreach ($fdata as $fitem)
         {
             $this->validation_pattern[$fitem['table'] . '.' . $fitem['field']] = $fitem['value'] ?? $pattern;
         }
         return $this;
     }
     ...
     public function modal($columns = '', $icon = false)
     {
         ...
         foreach ($fdata as $fitem)
         {
             $this->modal[$fitem['table'] . '.' . $fitem['field']] = $fitem['value'] ?? $icon;
         }
         return $this;
     }
     public function column_class($columns = '', $class = '')
     {
         ...
         foreach ($fdata as $fitem)
         {
             $this->column_class[$fitem['table'] . '.' . $fitem['field']][] = $fitem['value'] ?? $class;
         }
         return $this;
     }
     ...
     public function field_tooltip($fields = '', $tooltip = '', $icon = false)
     {
         if ($fields && $tooltip)
         {
             ...
             foreach ($fdata as $fitem)
             {
                 $this->field_tooltip[$fitem['table'] . '.' . $fitem['field']] = array('tooltip' => $fitem['value'] ?? $tooltip, 'icon' => $icon);
             }
         }
         return $this;
     }
     ...
     public function get_var($name = null)
     {
         if ($name)
         {
             return $this->custom_vars[$name] ?? false;
         }
         else
         {
             return false;
     ...
     public function column_tooltip($fields = '', $tooltip = '', $icon = false)
     {
         if ($fields && $tooltip)
         {
             ...
             foreach ($fdata as $fkey => $fitem)
             {
                 $this->column_tooltip[$fkey] = array('tooltip' => $fitem['value'] ?? $tooltip, 'icon' => $icon);
             }
     ...
     protected function render_custom_csv()
     {
         ...
         $table_name = $this->_clean_file_name(trim(html_entity_decode((string) $this->table_name, ENT_QUOTES, 'utf-8')));
         ...
         while ($row = $db->result->fetch_assoc()) // low level result process, saves memory
         {
             ...
             {
                 $out[] = htmlspecialchars_decode(strip_tags((string) $this->_render_export_item($field, $row[$field], $row['primary_key'], $row)),
                     ENT_QUOTES);
             }
     ...
     {
         $query = preg_replace('/\s+/u', ' ', (string) $this->query);
         ...
         $tmp = preg_replace_callback('/\( (?> [^)(]+ | (?R) )+ \)/xui', $this->query_params_callback(...), $query);
     ...
     {
         return preg_replace('/./Uui', '*', (string) $matches[0]);
     }
     ...
     {
         $field = str_replace('`', '', (string) $this->_get('field'));
         if (!$field)
             exit();
         ...
         {
             header("Content-type: application/octet-stream");
             header("Content-Disposition: attachment; filename=\"" . ($settings['filename'] ?? 'binary_data') . "\"");
             header("Content-Transfer-Encoding: binary");
         }
         else
         {
             if (trim(strtolower(strrchr((string) $path, '.')), '.') == 'pdf')
             {
                 header("Content-type: application/pdf");
                 header("Content-Disposition: inline; filename=\"" . ($settings['filename'] ?? $image) . "\"");
             }
             else
             {
                 header("Content-type: application/octet-stream");
                 header("Content-Disposition: attachment; filename=\"" . ($settings['filename'] ?? $image) .
                     "\"");
             }
     ...
     protected function _post($field = '', $default = false, $filter = false)
     {
         if (isset($_POST['xcrud'][$field]))
         {
             ...
             {
                 return match ($filter) {
                     'key' => str_replace('`', '', $xss ? $xss->xss_clean($_POST['xcrud'][$field]) : $_POST['xcrud'][$field]),
                     'int' => (int)$_POST['xcrud'][$field],
                     'trim' => trim($xss ? $xss->xss_clean($_POST['xcrud'][$field]) : $_POST['xcrud'][$field]),
                     'base64' => $xss ? $xss->xss_clean($this->fieldname_decode($_POST['xcrud'][$field])) : $this->fieldname_decode($_POST['xcrud'][$field]),
                     default => $xss ? $xss->xss_clean($_POST['xcrud'][$field]) : $_POST['xcrud'][$field],
                 };
             }
             else
             {
     ...
     protected function _get($field = '', $default = false, $filter = false)
     {
         if (isset($_GET['xcrud'][$field]))
         {
             ...
             {
                 return match ($filter) {
                     'key' => str_replace('`', '', $xss ? $xss->xss_clean($_GET['xcrud'][$field]) : $_GET['xcrud'][$field]),
                     'int' => (int)$_GET['xcrud'][$field],
                     'trim' => trim($xss ? $xss->xss_clean($_GET['xcrud'][$field]) : $_GET['xcrud'][$field]),
                     default => $xss ? $xss->xss_clean($_GET['xcrud'][$field]) : $_GET['xcrud'][$field],
                 };
             }
             else
             {
                 ...
                 if (isset($this->field_type[$key]))
                 {
                     switch ($this->field_type[$key])
                     {
                         case 'password':
                             ...
                             {
                                 $val = hash((string) $this->defaults[$key], (string) $val);
                             }
                             break;
                         case 'fk_relation': //
                             continue 2;
             ...
             if (isset($this->field_type[$key]))
             {
                 switch ($this->field_type[$key])
                 {
                     ...
                     case 'datetime':
                         if ($val !== '')
                         {
                             if (preg_match('/^\-{0,1}[0-9]+$/u', (string) $val))
                             {
         ...
         if (!$this->primary_val)
         {
             ...
             if ($this->alert_create)
             {
                 foreach ($this->alert_create as $alert)
                 {
                     ...
                     if (!$send_to or !preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/', (string) $send_to))
                         continue;
     ...
     protected function validate_postdata($postdata)
     {
         foreach ($postdata as $key => $val)
         {
             if (isset($this->validation_required[$key]) && mb_strlen((string) $val) < $this->validation_required[$key])
             {
                 $this->set_exception($key, 'validation_error', 'error');
             }
             elseif (isset($this->validation_pattern[$key]) && mb_strlen((string) $val) > 0)
             {
                 $reg = match ($this->validation_pattern[$key]) {
                     'email' => '/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/u',
                     'alpha' => '/^([a-z])+$/ui',
                     'alpha_numeric' => '/^([a-z0-9])+$/ui',
                     'alpha_dash' => '/^([-a-z0-9_-])+$/ui',
                     'numeric' => '/^[\-+]?[0-9]*\.?[0-9]+$/u',
                     'integer' => '/^[\-+]?[0-9]+$/u',
                     'decimal' => '/^[\-+]?[0-9]+\.[0-9]+$/u',
                     'point' => '/^[\-+]?[0-9]+\.{0,1}[0-9]*\,[\-+]?[0-9]+\.{0,1}[0-9]*$/u',
                     'natural' => '/^[0-9]+$/u',
                     default => '/' . $this->validation_pattern[$key] . '/u',
                 };
                 if (!preg_match($reg, (string) $val))
                 {
                     $this->set_exception($key, 'validation_error', 'error');
                 }
             }
     ...
     protected function call_exception($postdata = array())
     {
         ...
         {
             case 'upload':
                 return match ($this->_post('type')) {
                     'image' => $this->create_image($this->_post('field'), '') . $this->render_message(),
                     'file' => $this->create_file($this->_post('field'), '') . $this->render_message(),
                     default => self::error('Upload Error'),
                 };
                 break;
         }
     ...
     protected function subselect_query($name)
     {
         ...
         {
             $sql = preg_replace_callback('/\{(.+)\}/Uu', $this->subselect_callback(...), (string) $this->subselect[$name]);
     ...
     {
         if (strpos((string) $matches[1], '.'))
         {
             $tmp = explode('.', (string) $matches[1]);
         ...
         if ($this->search && ($this->task == 'list' or $this->task == 'print' or $this->task == 'csv' or $this->after == 'list'))
         {
             ...
             if ($this->column && isset($search_columns[$this->column]))
             {
                 ...
                 else
                 {
                     ...
                     switch ($this->field_type[$this->column])
                     {
                         ...
                         case 'date':
                         case 'time':
                             $format = match ($this->field_type[$this->column]) {
                                 'date' => 'Y-m-d',
                                 'time' => 'H:i:s',
                                 default => 'Y-m-d H:i:s',
                             };
             ...
             else
             {
                 ...
                 foreach ($search_columns as $key => $fitem)
                 {
                     ...
                     {
                         if (preg_match('/^[0-9\-\:\s]+$/', (string) $this->phrase))
                         {
     ...
     protected function _build_rel_where($name)
     {
         ...
         if ($this->fk_relation[$name]['rel_where'])
         {
             ...
             if (is_array($this->fk_relation[$name]['rel_where']))
             {
                 ...
                 {
                     $val = preg_replace_callback('/\{(.+)\}/Uu', $this->rel_where_callback(...), (string) $val);
                     $where_arr[] = $this->_field_from_where($field) . $this->_cond_from_where($field) . $db->escape($val);
                 }
             }
             else
             {
                 $where_arr[] = preg_replace_callback('/\{(.+)\}/Uu', $this->rel_where_callback(...), (string) $this->fk_relation[$name]['rel_where']);
             }
     ...
     protected function _build_rel_ins_where($name)
     {
         ...
         if ($this->fk_relation[$name]['add_data'])
         {
             ...
             {
                 $where_arr[] = preg_replace_callback('/\{(.+)\}/Uu', $this->rel_where_callback(...), (string) $this->fk_relation[$name]['add_data']);
             }
             return implode(' AND ', $where_arr);
         }
         else
     ...
     protected function _receive_post($task = false, $primary = false)
     {
         if (!$this->table_name && !$this->query)
             $this->table_name = $this->_humanize(mb_substr((string) $this->table, mb_strlen((string) $this->prefix)));
         if ($task)
         {
             switch ($task)
             {
     ...
     protected function _build_order_by()
     {
         if (count($this->order_by))
         {
             ...
             foreach ($this->order_by as $field => $direction)
             {
                 ...
                 elseif (isset($this->subselect[$field]) or isset($this->columns[$field]) or isset($this->no_select[$field]) or !strpos((string) $field,
     ...
     {
         if (preg_match('/^([A-Za-z]+)\((.+)\)/u', (string) $row['Type'], $matches))
         {
     ...
     protected function _render_list()
     {
         if (count($this->order_by))
         {
             $this->order_column = array_key_first($this->order_by);
             $this->order_direct = strtolower((string) $this->order_by[$this->order_column]);
         }
         else
         {
     ...
     protected function _render_details($mode)
     {
         if (count($this->order_by))
         {
             $order_direct = strtolower((string) reset($this->order_by));
             ...
             foreach ($this->fields as $field => $fitem)
             {
                 ...
                 else
                 {
                     ...
                     else
                     {
                         ...
                         {
                             $this->fields_output[$field]['field'] = str_ireplace('{value}', (string) $this->fields_output[$field]['field'], (string) $this->
             ...
             foreach ($this->inner_table_instance as $inst_name => $field)
             {
                 if (isset($this->result_row[$field]))
                 {
                     ...
                     $this->nested_rendered[$inst_name] =
                         '<div class="xcrud-nested-container xcrud-container"><div class="xcrud-ajax" id="xcrud-ajax-' . base_convert(random_int(0, mt_getrandmax()), 10,
     ...
     protected function _pagination($total, $start, $limit, $numpos = 10, $numlr = 2)
     {
         if ($total > $limit)
         {
             ...
             '"><a href="javascript:;" class="' . $this->theme_config('pagination_link') . ' xcrud-action" data-start="' . $limit1 . '">' . $i . '</a></li>';
             }
             }*/
             $html = '<ul class="' . $this->theme_config('pagination_container') . '" style="    float: right;">';
             if ($pages > $numpos)
             {
     ...
     protected function _cut($string, $field, $wordsafe = true, $dots = true)
     {
         ...
         $string = html_entity_decode((string) $string, ENT_QUOTES, Xcrud_config::$mbencoding);
     ...
     {
         return mb_convert_case(str_replace('_', ' ', (string) $text), MB_CASE_TITLE, Xcrud_config::$mbencoding);
     }
     protected function _regenerate_key()
     {
         switch ($this->task)
         {
             ...
             case 'unique':
                 break;
             default:
                 $this->key = sha1(microtime() . random_int(0, mt_getrandmax()));
                 break;
         }
     }
     ...
     protected function find_prev_task()
     {
         return match ($this->task) {
             'create', 'view', 'edit', 'list' => $this->task,
             '' => 'list',
             default => $this->before ? $this->before : 'list',
         };
     }
     ...
     protected function create_texteditor($name, $value = '', $tag = array())
     {
         ...
         $tag = $tag + array(
             ...
             'name' => $name,
             'id' => 'editor_' . base_convert(random_int(0, mt_getrandmax()), 10, 36));
     ...
     protected function create_file($name, $value = '', $tag = array(), $is_upload = false)
     {
         ...
         if ($value)
         {
             ...
             else
             {
                 ...
                 $ext = trim(strtolower(strrchr((string) $value, '.')), '.');
             }
             ...
             $out .= ($this->upload_config[$name]['text'] ?? $value);
     ...
     protected function upload_file_button($name, $value, $tag = array())
     {
         ...
         $attr = array(
             'id' => 'xfupl' . random_int(0, mt_getrandmax()),
             'value' => '',
             'type' => 'file',
             'data-type' => 'file',
             'data-field' => $name,
     ...
     protected function create_view_file($name, $value = '', $tag = array(), $is_upload = false)
     {
         ...
         if ($value)
         {
             ...
             $out .= ($this->upload_config[$name]['text'] ?? $value) . $this->close_tag('a');
             $out .= ' ' . $file_size;
             $this->close_tag('span');
         }
         else
     ...
     protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
     {
         ...
         if ($value)
         {
             ...
             $attr = array('src' => isset($this->upload_config[$name]['url']) ? $this->real_file_link($value, $this->upload_config[$name], true) :
                     $this->file_link($name, $this->primary_val, ($this->upload_config[$name]['detail_thumb'] ?? false), false,
                     $value), 'alt' => '');
     ...
     protected static function error($text = 'Error!'): never
     {
     ...
     {
         $type = $this->field_type[$this->_post('field')] ?? false;
         return match ($type) {
             'image' => $this->_remove_image(),
             'file' => $this->_remove_file(),
             default => self::error('Remove Error'),
         };
     }
     ...
     {
         [$srcWidth, $srcHeight, $type] = getimagesize($src_file);
         switch ($type)
         {
             case 1:
         ...
         if ($watermark)
         {
             [$water_w, $water_h, $water_type] = getimagesize($watermark);
     ...
     protected function _clean_file_name($txt)
     {
         $replace = array(
             ...
             'ū' => 'u',
             ' ' => '_');
         $txt = str_replace(array_keys($replace), array_values($replace), (string) $txt);
     ...
     {
         return number_format(strlen((string) $text) / 1024, 2, '.', ' ') . ' KB';
     }
     protected function _prepare_field($field)
     {
         preg_match_all('/([^<>!=]+)/u', (string) $field, $matches);
         preg_match_all('/([<>!=]+)/u', (string) $field, $matches2);
         return '`' . trim((string) $matches[0][0]) . '`' . ($matches2[0] ? implode('', $matches2[0]) : '=');
     }
     protected function _field_from_where($field)
     {
         return preg_replace('/\s*[<>!=]+\s*$/u', '', (string) $field);
     }
     protected function _cond_from_where($field)
     {
         if (preg_match('/\s*([<>!=]+)\s*$/u', (string) $field, $matches))
         {
             return $matches[1];
         }
         else
     ...
     {
         if (preg_match('/\s*[!]+\s*$/u', (string) $field))
         {
             return ' NOT IN';
         }
         else
     ...
     protected function _prepare_field_in($field)
     {
         preg_match_all('/([^!]+)/u', (string) $field, $matches);
         preg_match_all('/([!]+)/u', (string) $field, $matches2);
         return '`' . trim((string) $matches[0][0]) . '`' . ($matches2[0] ? ' NOT IN' : ' IN');
     }
     ...
     protected function _compare($val1, $operator, $val2)
     {
         return match ($operator) {
             ...
             '<=' => ($val1 <= $val2) ? true : false,
             '!=' => ($val1 != $val2) ? true : false,
             '^=' => (mb_strpos((string) $val1, (string) $val2, 0, Xcrud_config::$mbencoding) === 0) ? true : false,
             '$=' => (mb_strpos((string) $val1, (string) $val2, 0, Xcrud_config::$mbencoding) == (mb_strlen((string) $val1, Xcrud_config::$mbencoding) -
                 mb_strlen((string) $val2, Xcrud_config::$mbencoding))) ? true : false,
             '~=' => (mb_strpos((string) $val1, (string) $val2, 0, Xcrud_config::$mbencoding) !== false) ? true : false,
             default => false,
         };
     }
     ...
     protected function _render_list_item($field, $value, $primary_val, $row)
     {
         ...
         if (isset($this->field_type[$field]))
         {
             switch ($this->field_type[$field])
             {
                 ...
                 case 'image':
                     if ($value)
                     {
                         ...
                         else
                         {
                             ...
                             $out .= $this->upload_config[$field]['text'] ?? $value;
                             $out .= $this->close_tag('a');
                         }
                     }
                 ...
                 case 'texteditor':
                     ...
                     {
                         $out .= nl2br((string) $this->_cut($value, $field));
                     }
                     break;
                 default:
         ...
         {
             $out = str_ireplace('{value}', $out, (string) $this->column_pattern[$field]);
     ...
     protected function make_mailto($txt)
     {
         if ($this->emails_label)
             return preg_replace('/([A-Za-z0-9_\-\.]+)\@([A-Za-z0-9_\-\.]+)\.([A-Za-z]{2,4})/',
                 '<a target="_blank" href="mailto:$1@$2.$3">' . $this->emails_label['text'] . '</a>', (string) $txt);
         else
             return preg_replace('/([A-Za-z0-9_\-\.]+)\@([A-Za-z0-9_\-\.]+)\.([A-Za-z]{2,4})/',
                 '<a target="_blank" href="mailto:$1@$2.$3">$1@$2.$3</a>', (string) $txt);
     }
     protected function make_links($txt)
     {
         if ($this->links_label)
             ...
             return preg_replace('/(http:\/\/|https:\/\/)([^\s]+)/u', '<a target="_blank" href="$1$2">$1$2</a>', (string) $txt);
     }
     ...
     protected function _render_export_item($field, $value, $primary_val, $row)
     {
         ...
         if (isset($this->relation[$field]))
         {
             $value = strip_tags((string) $row['rel.' . $field]);
         }
         if (isset($this->column_callback[$field]))
         {
             $path = $this->check_file($this->column_callback[$field]['path'], 'column_callback');
             ...
             if (is_callable($this->column_callback[$field]['callback']) && $row)
             {
                 $value = strip_tags((string) call_user_func_array($this->column_callback[$field]['callback'], array(
                     $value,
                     $field,
                     $primary_val,
                     $row,
     ...
     protected function render_sum_item($field)
     {
         if (isset($this->sum_row[$field]))
         {
             ...
             {
                 return str_replace('{value}', $this->_render_list_item($field, $this->sum_row[$field], 0, null), (string) $this->sum[$field]['custom']);
             }
             else
             {
     ...
     public static function check_url($url, $scr_url = false)
     {
         ...
         $scheme = (!isset($_SERVER['HTTPS']) or !$_SERVER['HTTPS'] or strtolower((string) $_SERVER['HTTPS']) == 'off' or strtolower((string) $_SERVER['HTTPS']) ==
             'no') ? 'http://' : 'https://';
         // some troubles with sym links between private and public
         $doc_root = trim(str_replace('\\', '/', str_replace(array('/public_html', '/private_html'), '', (string) $_SERVER['DOCUMENT_ROOT'])),
             '/');
     ...
     protected function file_link($field, $primary_val, $thumb = false, $crop = false, $filename = false)
     {
         ...
         $params = array('xcrud' => array(
                 ...
                 'key' => $this->key,
                 'task' => 'file',
                 'rand' => base_convert(sha1($this->instance_name . ($filename ? $filename : random_int(0, mt_getrandmax()))), 10, 36)));
         if ($thumb !== false)
         {
     ...
     protected function send_email($to, $subject = '(No subject)', $message = '', $cc = array(), $html = true)
     {
         ...
         mail((string) $to, '=?UTF-8?B?' . base64_encode((string) $subject) . '?=', (string) $message, $header);
     }
     ...
     {
         $langtext = mb_convert_case((string) $text, MB_CASE_LOWER, Xcrud_config::$mbencoding);
         return htmlspecialchars((self::$lang_arr[$langtext] ?? $text), ENT_QUOTES,
             Xcrud_config::$mbencoding);
     }
     protected function theme_config($text = '')
     {
         $text = mb_convert_case((string) $text, MB_CASE_LOWER, Xcrud_config::$mbencoding);
         return htmlspecialchars(($this->theme_config[$text] ?? ''), ENT_QUOTES, Xcrud_config::
             $mbencoding);
     }
     protected function _thumb_name($name, $marker)
     {
         return substr_replace((string) $name, (string) $marker, strrpos((string) $name, '.'), 0);
     }
     ...
     public static function load_js()
     {
         ...
         $out .= '
             ...
             <!--
            	...
            	var xcrud_config = ' . json_encode($config, JSON_THROW_ON_ERROR) . ';
     ...
     protected function check_file($path, $func_name)
     {
         ...
         [$root_folder] = explode('/', trim(XCRUD_PATH, '/'), 2);
         [$root_path] = explode('/', trim($path, '/'), 2);
         if (str_contains($path, '../') or $root_folder != $root_path)
     ...
     protected function additional_columns($fields = '')
     {
         if ($fields)
         {
             ...
             foreach ($fdata as $key => $fitem)
             {
                 if (!isset($this->subselect[$key]) && $fitem['field'] != 'value' && mb_substr_count((string) $key, '.') < 2)
                 {
     ...
     {
         $found = preg_match_all('/\{([^\}]+)\}/u', (string) $text, $matches);
         if ($found)
         {
             switch ($mode)
             {
     ...
     protected function replace_text_variables($value, array $data, $safety = false, $null_if_empty = false)
     {
         if (!is_array($value) && !Xcrud_config::$performance_mode && $value)
         {
             foreach ($data as $key => $val)
             {
                 ...
                 if (!is_array($val))
                 {
                     $value = str_ireplace('{' . $key . '}', $safety ? $this->html_safe($val) : $val, (string) $value);
                     if ($tbl == $this->table)
     ...
     protected function send_http_request($url, $data, $method, $return_result = false)
     {
         ...
         {
             case 'get':
                 $ch = curl_init($path . ((mb_strpos((string) $path, '?', 0, Xcrud_config::$mbencoding) === false) ? '?' : '&') . $data);
                 break;
             case 'post':
                 $ch = curl_init($path);
         ...
         'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
     {
         ...
         if (isset($this->field_tabs[$mode]) or $this->default_tab !== false)
         {
             ...
             {
                 $tabid = 'tabid_' . base_convert(random_int(0, mt_getrandmax()), 10, 36);
         ...
         if ($this->search)
         {
             if ($this->column)
             {
                 switch ($this->field_type[$this->column])
                 {
                     ...
                     case 'date':
                     case 'time':
                         $out .= $this->single_tag($tag, '', array('name' => 'phrase][from', 'value' => ($this->
                                 phrase['from'] ?? '')));
                         $out .= $this->single_tag($tag, '', array('name' => 'phrase][to', 'value' => ($this->
                                 phrase['to'] ?? '')));
                         break;
                     default:
         ...
         if ($this->result_list)
         {
             foreach ($this->result_list as $key => $row)
             {
                 ...
                 if ($this->is_numbers)
                 {
                   ...
                   $out .= $this->open_tag($item, 'xcrud-num', $this->_cell_attrib(false, false, $this->order_column, $row, false, $row_color, $row_class)) . $this->close_tag($item);
         ...
         if ($this->sum && $this->result_list)
         {
             ...
             foreach ($this->columns as $field => $fitem)
             {
                 ...
                 $out .= $this->open_tag($item, isset($this->sum[$field]) ? $this->sum[$field]['class'] : '', $this->_cell_attrib($field,
                     $this->sum[$field] ?? null, $this->order_column, $this->sum, true));
     ...
     protected function check_file_folders($field)
     {
         ...
         if (isset($settings['thumbs']) && is_array($settings['thumbs']))
         {
             foreach ($settings['thumbs'] as $thumb)
             {
                 if (isset($thumb['folder']) && !is_dir($path . '/' . trim((string) $thumb['folder'], '/')))
                 {
                     $this->create_file_folders($path . '/' . trim((string) $thumb['folder'], '/'));
                 }
             }
         }
     }
     ...
     protected function get_thumb_path($imgname, $field, $thumb_array)
     {
         ...
         {
             $path .= '/' . trim((string) $thumb_array['folder'], '/');
         }
         $marker = $thumb_array['marker'] ?? '';
         return $path . '/' . $this->_thumb_name($imgname, $marker);
     }
     protected function safe_file_name($file, $field)
     {
         $ext = strtolower(strrchr((string) $file['name'], '.'));
         if (isset($this->upload_config[$field]['not_rename']) && $this->upload_config[$field]['not_rename'] == true)
             ...
             $filename = base_convert(str_replace(' ', '', microtime()) . random_int(0, mt_getrandmax()), 10, 36) . $ext;
         return $filename;
     }
     protected function get_ext($filename)
     {
         return strtolower((string) (strrchr((string) $filename, '.') + 1));
     }
     protected function save_file_to_tmp($file, $filename, $field)
     {
         $filename = substr((string) $filename, 0, strrpos((string) $filename, '.')) . '.tmp';
         $file_path = $this->get_image_folder($field) . '/' . $filename;
     ...
     protected function get_filename_noconfict($filename, $field)
     {
         ...
         {
             $filename = substr_replace((string) $filename, '_' . base_convert(time() . random_int(0, mt_getrandmax()), 10, 36), strrpos((string) $filename, '.'), 0);
         }
         return $filename;
     }
     ...
     {
         $tmp_filename = substr((string) $filename, 0, strrpos((string) $filename, '.')) . '.tmp';
     ...
     protected function mysql2datetime($date)
     {
         ...
         {
             if (!preg_match('/^\-{0,1}[0-9]+$/u', (string) $date))
             {
     ...
     protected function mysql2time($date)
     {
         if ($date)
         {
             ...
             {
                 [$tmp, $date] = explode(' ', (string) $date, 2);
             }
             $d = explode(':', (string) $date);
     ...
     protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
     {
         ...
         if ($this->is_title)
         {
             ...
             match ($mode) {
                 'create' => $out .= $this->is_rtl ? '<small>' . $this->lang('add') . ' - </small>' . $this->table_name : $this->table_name .
                     '<small> - ' . $this->lang('add') . '</small>',
                 'edit' => $out .= $this->is_rtl ? '<small>' . $this->lang('edit') . ' - </small>' . $this->table_name : $this->table_name .
                     '<small> - ' . $this->lang('edit') . '</small>',
                 'view' => $out .= $this->is_rtl ? '<small>' . $this->lang('view') . ' - </small>' . $this->table_name : $this->table_name .
                     '<small> - ' . $this->lang('view') . '</small>',
                 default => $out .= $this->is_rtl ? '<small>' . $this->get_table_tooltip() . '</small>' . $this->table_name : $this->table_name .
     ...
     {
         return 'id="xc_' . base_convert(time() + random_int(0, mt_getrandmax()), 10, 36) . '"';
     }


     public function decrypt($text, $iv) {
         [$encrypted_data, $iv] = explode('::', base64_decode((string) $text), 2);
      ...
      public function encrypt($payload) {
       ...
       $encrypted = openssl_encrypt($payload, 'aes-256-cbc', $alt_encription_key, 0, $iv);
     ...
     protected function cast_number_format($number, $field, $edit = false)
     {
         ...
         $number = preg_replace('/^(.*)[\.\,' . preg_quote((string) $this->field_attr[$field]['point'], '/') . ']+([^\.\,' . preg_quote((string) $this->
             field_attr[$field]['point'], '/') . ']*)$/ui', '$1' . $loc_point . '$2', (string) $number);
 ...
 {
     public function __construct(private array $postdata, private $xcrud)
     {
         unset($postdata);
     }
     ...
     public function get($name)
     {
         $fdata = $this->xcrud->_parse_field_names($name, 'Xcrud_postdata');
         $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
         $value = ($this->postdata[$fname] ?? false);
