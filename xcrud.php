...
// includo in xcrud i file di configurazione al database
...
class Xcrud
{
    ...
    protected function __construct()
    {
		...
		$this->is_delete_all = Xcrud_config::$enable_deleteall;
	...
	public function unset_delete_all($bool = true)
    {
        $this->is_delete_all = !(bool)$bool;
        return $this;
    }
 ...
 if ($this->is_delete_all)
                {
                     $out .= '<div class="col-md-12 well">
<span class="rows_selected" id="select_count">0 Selezionati</span>
<a type="button" id="delete_records"  data_action="delete" data_table="'. $this->table . '"  class="btn btn-danger pull-right">Cancella Record Selezionati</a>
<a type="button" id="export_csv" data_action="export" data_table="'. $this->table . '" class="btn btn-success pull-right">Esporta CSV Selezionati</a>
<a type="button" id="print_data" data_action="print" data_table="'. $this->table . '" class="btn btn-warning pull-right">Stampa CSV Selezionati</a>
<a type="button" id="edit_data" data_action="edit" data_table="'. $this->table . '" class="btn btn-primary pull-right">Modifica Selezionati</a>
</div>';
                }
                else
                {
        ...
        if ($this->is_numbers)
        {
              if ($this->is_delete_all)
                {
                   $out .= $this->open_tag($item, 'xcrud-num') . '<input type="checkbox" id="select_all">' . $this->close_tag($item);  
                }
                else
                {
    ...
    protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        if ($this->result_list)
        {
            foreach ($this->result_list as $key => $row)
            {
				...
				if ($this->is_delete_all)
                {
                   $out .= $this->open_tag($item, 'xcrud-num', $this->_cell_attrib(false, false, $this->order_column, $row, false, $row_color, $row_class)) . '<input type="checkbox" class="emp_checkbox" data-emp-id="'. $row['primary_key'] . '" value="'. $key . '">'. $this->close_tag($item);
                }
                else
                {
