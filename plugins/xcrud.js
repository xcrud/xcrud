...
$("#delete_records").on("click", function (e) {
  ...
  if (employee.length <= 0) {
    ...
    WRN_PROFILE_DELETE =
      ...
      (employee.length > 1 ? "questi" : "questo") +
      " record ?";
...
var Xcrud = {
  ...
  request: function (container, data, success_callback) {
    ...
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        jQuery(document).trigger("xcrudafterrequest", [
          container,
          data,
          status,
        ]);
  ...
  new_window_request: function (container, data) {
    ...
    var w = window.open(
      "",
      "Xcrud_request",
      "scrollbars,resizable,height=400,width=600",
    );
    w.document.open();
  ...
  show_progress: function (container) {
    jQuery(container)
      .closest(".xcrud")
      .find(".xcrud-overlay")
      .width(jQuery(container).closest(".xcrud-container").width())
      .stop(true, true)
      .fadeTo(300, 0.6);
  },
  ...
  list_data: function (container, element) {
    ...
    jQuery(container)
      .find('.xcrud-input:not([type="checkbox"],[type="radio"],[disabled])')
      .each(function () {
        if (Xcrud.check_container(this, container)) {
          ...
          if (
            validation &&
            required &&
            !Xcrud.validation_required(val, required)
          ) {
            ...
            validation &&
            pattern &&
            !Xcrud.validation_pattern(val, pattern)
          ) {
            Xcrud.validation_error = 1;
            jQuery(this).addClass("validation-error");
          } else {
    ...
    jQuery(container)
      .find('.xcrud-input[data-type="checkboxes"]:not([disabled])')
      .each(function () {
        if (data.postdata[jQuery(this).attr("name")] === undefined) {
          data.postdata[jQuery(this).attr("name")] = "";
        }
        if (
          Xcrud.check_container(this, container) &&
          ...
          if (!data.postdata[jQuery(this).attr("name")]) {
            ...
            data.postdata[jQuery(this).attr("name")] +=
              "," + Xcrud.prepare_val(this);
          }
        }
      });
    jQuery(container)
      .find('.xcrud-input[type="radio"]:not([disabled])')
      .each(function () {
        if (
          Xcrud.check_container(this, container) &&
          jQuery(this).prop("checked")
    ...
    jQuery(container)
      .find('.xcrud-input[data-type="bool"]:not([disabled])')
      .each(function () {
        if (Xcrud.check_container(this, container)) {
          data.postdata[jQuery(this).attr("name")] = jQuery(this).prop(
            "checked",
          )
            ? 1
            : 0;
    ...
    jQuery(container)
      .find(".xcrud-searchdata.xcrud-search-active")
      .each(function () {
        if (Xcrud.check_container(this, container)) {
          data[jQuery(this).attr("name")] = Xcrud.prepare_val(this);
        }
  ...
  check_container: function (element, container) {
    return jQuery(element).closest(".xcrud-ajax").attr("id") ==
    ...
    switch (jQuery(element).data("type")) {
      ...
      case "time":
        if (jQuery(element).val()) {
          ...
          return d
            ? Math.round(d.getTime() / 1000) - d.getTimezoneOffset() * 60
            : "";
        } else return "";
        break;
      default:
  ...
  change_filter: function (type, container, fieldname) {
    jQuery(container)
      .find(".xcrud-searchdata")
      .hide()
      .removeClass("xcrud-search-active");
    var name_selector = "";
    switch (type) {
      case "datetime":
      case "timestamp":
    ...
    jQuery(container)
      .find(
        '.xcrud-searchdata[data-fieldtype="' + fieldtype + '"]' + name_selector,
      )
      .show()
      .addClass("xcrud-search-active");
    if (fieldtype == "date") {
  ...
  init_datepicker_range: function (type, container) {
    jQuery(container)
      .find(
        ".xcrud-datepicker-from.hasDatepicker,.xcrud-datepicker-to.hasDatepicker",
      )
      .datepicker("destroy");
    ...
    switch (type) {
      ...
      case "timestamp":
        ...
        datepicker_config.onClose = function (selectedDate) {
          jQuery(container)
            .find(".xcrud-datepicker-from")
            .datetimepicker("option", "maxDate", selectedDate);
        };
        datepicker_config.onSelect = datepicker_config.onClose;
        jQuery(container)
          .find(".xcrud-datepicker-to")
          .datetimepicker(datepicker_config);
        // from
        datepicker_config.maxDate = jQuery(container)
          .find(".xcrud-datepicker-to")
          .val();
        datepicker_config.onClose = function (selectedDate) {
          jQuery(container)
            .find(".xcrud-datepicker-to")
            .datetimepicker("option", "minDate", selectedDate);
        };
        datepicker_config.onSelect = datepicker_config.onClose;
        jQuery(container)
          .find(".xcrud-datepicker-from")
      ...
      case "time":
        jQuery(container)
          .find(".xcrud-datepicker-from,.xcrud-datepicker-to")
          .timepicker(datepicker_config);
        break;
    }
  ...
  init_texteditor: function (container) {
    var elements = jQuery(container).find(
      ".xcrud-texteditor:not(.editor-loaded)",
    );
  ...
  show_crop_window: function (crop_img, container) {
    var upl_container = jQuery(container)
      .find("img.xcrud-crop")
      .closest(".xcrud-upload-container");
    jQuery(crop_img).dialog({
      ...
      open: function (event, ui) {
        Xcrud.load_image(crop_img.attr("src"), function (imageObject) {
          ...
          var left = Math.round(
            (jQuery(window).width() - cropset.boxWidth - 10) / 2,
          );
          var top = Math.round(
            (jQuery(window).height() - cropset.boxHeight - 75) / 2,
          );
  ...
  get_coordinates: function (c) {
    ...
    jQuery(".xcrud")
      .find("input.xrud-crop-data[name=x2]")
      .val(Math.round(c.x2));
    jQuery(".xcrud")
      .find("input.xrud-crop-data[name=y2]")
      .val(Math.round(c.y2));
  ...
  check_fixed_buttons: function () {
    jQuery(".xcrud").each(function () {
      if (
        jQuery(this).find(".xcrud-list:first").width() >
        jQuery(this).find(".xcrud-list-container:first").width()
      ) {
        var w = jQuery(this)
          .find(".xcrud-actions:not(.xcrud-fix):first")
          .width();
        ...
        jQuery(this)
          .find(
            ".xcrud-list:first .xcrud-actions.xcrud-fix:not(.xcrud-actions-fixed)",
          )
          .addClass("xcrud-actions-fixed");
      } else
        jQuery(this)
          .find(".xcrud-list:first .xcrud-actions")
          .removeClass("xcrud-actions-fixed");
    });
  },
  block_query: {},
  depend_init: function (container) {
    ...
    jQuery.map(dependencies, function (val, key) {
      window.setTimeout(function () {
        jQuery(container)
          .find('.xcrud-input[name="' + val + '"]:not([data-depend])')
  ...
  depend_query: function (data, depend_on, container) {
    ...
    jQuery.ajax({
      ...
      success: function (input) {
        jQuery(container)
          .find('.xcrud-input[name="' + data.name + '"]')
          .replaceWith(input);
        window.setTimeout(function () {
    ...
    if (infowindow) {
      google.maps.event.addListener(marker, "click", function () {
        ...
        infoWindow.setContent(
          '<p class="xcrud-infowinow">' + infowindow + "</p>",
        );
  ...
  map_init: function (container) {
    ...
    jQuery(container)
      ...
      .each(function () {
        ...
        var map = Xcrud.create_map(
          cont,
          point,
          jQuery(cont).data("zoom"),
          "ROADMAP",
        );
        var marker = Xcrud.place_marker(
          map,
          point,
          jQuery(cont).data("draggable"),
          jQuery(cont).data("text"),
          point_field,
        );
  ...
  bootstrap_modal: function (header, content) {
    ...
    jQuery("body").append(
      '<div id="xcrud-modal-window" class="modal"><div class="modal-dialog"><div class="modal-content"></div></div></div>',
    );
    jQuery("#xcrud-modal-window .modal-content").html(
    ...
    jQuery("#xcrud-modal-window .modal-content").append(
      '<div class="modal-body">' + content + "</div>",
    );
    jQuery("#xcrud-modal-window").modal();
    jQuery('#xcrud-modal-window [data-dismiss="modal"]').on(
      "click",
      function () {
        jQuery("#xcrud-modal-window").modal("hide");
        if (jQuery(".simplemodal-close").size()) {
          // joomla trick
...
jQuery(document).on("ready xcrudreinit", function () {
  ...
  if ($(".xcrud").size()) {
      ...
      if ($(this).val()) {
        $(container)
          ...
          .datepicker(
            "setDate",
            new Date(
              ($(this).find("option:selected").data("from") +
        ...
        $(container)
          ...
          .datepicker(
            "setDate",
            new Date(
              ($(this).find("option:selected").data("to") +
                new Date().getTimezoneOffset() * 60) *
                1000,
            ),
          );
    ...
    $(".xcrud-ajax").each(function () {
      ...
      Xcrud.init_datepicker_range(
        $(this).find(".xcrud-columns-select option:selected").data("type"),
        this,
      );
...
jQuery(document).on("xcrudafterrequest", function (event, container) {
  ...
  Xcrud.init_datepicker_range(
    jQuery(container)
      .find(".xcrud-columns-select option:selected")
      .data("type"),
    container,
  );
  ...
  print_window: function (print_win, xcrud) {
    ...
    jQuery.ajax({
      ...
      success: function (out) {
        ...
        jQuery(xcrud)
          .find(".xcrud-data[name=key]:first")
          .val(
            jQuery(print_win.document)
              .find(".xcrud-data[name=key]:first")
              .val(),
          );
